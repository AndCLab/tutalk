...
const ConversationItem = ({conversation, selectedConversation=null, online=null}) => {
    ...
    return (
        ...
        preserveState
        className={"conversation-item flex items-center gap-2 p-2 text-gray-100 transition-all cursor-pointer border-1-4 border-emerald-900 hover:bg-gray-100"
        + classes + (conversation.is_user && currentUser.is_admin ? " pr-2" : " pr-4")}>
            ...
            <div className={`flex-1 text-xs max-w-full overflow-hidden` + (conversation.is_user && conversation.blocked_at ? "opacity-50" : "")}>
                <div className="flex gap-1 justify-between items-center">
                    ...
                    {conversation.last_message_date && (<span className="text-nowrap opacity-50 text-gray-400">{ formatMessageDateShort(conversation.last_message_date) }</span>)}
                </div>
                {conversation.last_message && (
                    <p className="text-xs text-nowrap overflow-hidden text-ellipsis text-gray-400">
                        {conversation.last_message}
                    </p>
                )}
            </div>
